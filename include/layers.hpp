...
#define BCE_REORDER 3
#define BCE_REORDER_INV 4
#define BCE_MATMUL 5
#define BCE_MATMUL_INV 6
#define BCE_SUM 7
#define BCE_NORM 8
                           ...
                           dnnl::engine eng)
{
    ...
    // 4) Reorder y_true from nc to cn {batch, 1} -> {1, batch}
    ...
    // 5) Perform elementwise linear on y_true_reordered with alpha = -1; beta = 1 ie. 1-y_true
    ...
    // 6) Perform matmul(y_true_reordered, log(y_hat))
    ...
    // 7) Perform matmul(1-y_true_reordered, log(1-y_hat))
    ...
    // 8) Sum obtained values (should be scalars)
    ...
    // 9) Divide by -N using elementwise linear
